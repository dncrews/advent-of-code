...
function getHandler(line) {
  return handlers[line.match(/^(toggle)|(turn on)|(turn off)/)[0]];
}
...
function changeEm(handler, range) {
  db = db.map((list, row) => {
    return list.map((val, column) => {
      if (row >= range.y0 && row <= range.y1 && column >= range.x0 && column <= range.x1) {
        return handler(val);
      }
      return val;
    });
...
input.split('\n').forEach((line) => {
  let handler = getHandler(line);
  let range = getRange(line);
  changeEm(handler, range);
});

let total = db.reduce((total, row) => {
  return total + row.reduce((total, val) => {
    return total + val;
  }, 0);
}, 0);
